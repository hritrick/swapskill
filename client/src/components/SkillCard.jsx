...
export default function SkillCard({ skill }) {
  ...
  const { openProposal } = useContext(SwapsContext);
  const { deleteSkill } = useContext(SkillsContext);

  const ownerName = skill.by || skill.owner?.name || 'Community Member';
  const ownerId = skill.ownerId || skill.owner?._id || skill.owner;
  ...
  return (
    <div className="skill-card bg-paper border border-line rounded-xl p-5">
      <div className="flex justify-between items-start">
        ...
        <span className="text-ink/40 text-xs">by {ownerName}</span>
      </div>
      ...
      <div className="flex items-center justify-between mt-4">
        ...
        {user && (user._id === (ownerId?.toString() || ownerId) || user.role === 'admin') && (
          <button
            onClick={() => {
